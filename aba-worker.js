...
self.onmessage=async({data})=>{const{action='parse',file,index,total,token,site,storageFile,reports,terms,primary,comparison,filters,sorts,page,pageSize,queryKey}=data;try{
  ...
  if(action==='hash-file'){const sha=new Sha256(),reader=file.stream().getReader();let bytes=0,lastPercent=-1;while(true){const{value,done}=await reader.read();if(done)break;sha.update(value);bytes+=value.length;const percent=Math.floor(bytes/Math.max(1,file.size)*100);if(percent!==lastPercent){lastPercent=percent;self.postMessage({type:'hash-progress',token,index,total,percent,bytes});}}self.postMessage({type:'hash-result',token,index,total,fileHash:sha.hex()});return;}
