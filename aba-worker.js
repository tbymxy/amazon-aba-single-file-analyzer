...
const TREND_MAGIC = 0x41424154;
const MAX_TREND_BUFFERS = 2;
...
function reportPeriod(firstRecord){const source=firstRecord.join(' ').replace(/^\uFEFF/,'');const match=/(?:周|週|week|woche|semaine|settimana|semana)\s*(\d{1,2})\s*\|\s*(\d{4}-\d{2}-\d{2})\s*-\s*(\d{4}-\d{2}-\d{2})/i.exec(source);if(!match)return null;return{weekNumber:Number(match[1]),startDate:match[2],endDate:match[3],label:`周 ${Number(match[1])} | ${match[2]} - ${match[3]}`};}
const includesAny=(value,aliases)=>aliases.some(alias=>value.includes(alias));
function fieldIndexes(headers){const fields=headers.map((raw,index)=>({index,value:normalize(raw)}));const rankAliases=['搜索频率排名','search frequency rank','suchfrequenzrang','rang der suchhäufigkeit','classement de fréquence de recherche','classifica della frequenza di ricerca','clasificación de frecuencia de búsqueda','検索頻度ランク'];const termAliases=['搜索词','search term','suchbegriff','terme de recherche','termine di ricerca','término de búsqueda','検索用語'];const clickAliases=['点击份额','click share','klickanteil','part de clics','percentuale di clic','quota clic','porcentaje de clics','cuota de clics','クリックシェア'];const conversionAliases=['转化贡献','conversion share','conversion-anteil','anteil an conversions','part de conversion','quota di conversione','percentuale di conversione','porcentaje de conversiones','cuota de conversión','コンバージョンシェア'];const rank=fields.find(f=>rankAliases.includes(f.value));const term=fields.find(f=>termAliases.includes(f.value));const click=fields.filter(f=>includesAny(f.value,clickAliases)).slice(0,3);const conversion=fields.filter(f=>includesAny(f.value,conversionAliases)).slice(0,3);if(!rank||!term||click.length<3||conversion.length<3)throw new Error('无法识别 ABA 核心字段，请确认选择的是热门搜索词简单报告 CSV。');return{rank:rank.index,term:term.index,click:click.map(f=>f.index),conversion:conversion.map(f=>f.index)};}
...
async function importReport(file,site,token,index,total,precomputedHash){const directory=await reportDirectory();if(!directory)throw new Error('当前浏览器不支持本地持久化存储。');const safe=String(token).replace(/[^a-zA-Z0-9-]/g,'')+'-'+index;const storageFile=`${site}_import_${safe}.aba2`,trendFile=`${site}_import_${safe}.trend.bin`;const writer=await AnalysisWriter.create(directory,storageFile);const triples=new Uint32Triples();const sha=precomputedHash?null:new Sha256();let period=null,indexes=null,sourceRow=0,lastRank=0,readBytes=0,lastProgress=0;let csv;
  try{
    ...
    while(true){const {value,done}=await reader.read();if(done)break;if(sha)sha.update(value);readBytes+=value.length;csv.feed(textDecoder.decode(value,{stream:true}));await writer.flush();const percent=Math.min(99,Math.floor(readBytes/Math.max(1,file.size)*100));if(percent!==lastProgress){lastProgress=percent;self.postMessage({type:'progress',token,index,total,percent,count:writer.count,bytes:readBytes});}}
    csv.feed(textDecoder.decode());csv.finish();await writer.flush();if(!period)throw new Error('无法从 CSV 第一行识别周数与日期。');if(!indexes)throw new Error('未找到 ABA 表头。');if(!writer.count)throw new Error('报告中没有可用的搜索词数据。');const hash=precomputedHash||sha.hex();await writer.finish();await saveTrendArrays(directory,trendFile,triples,token);return{period,fileHash:hash,storageFile,trendFile,rowCount:writer.count,storageVersion:2};
...
function cacheTrendBuffer(fileName,buffer){if(trendBuffers.has(fileName))trendBuffers.delete(fileName);trendBuffers.set(fileName,buffer);while(trendBuffers.size>MAX_TREND_BUFFERS)trendBuffers.delete(trendBuffers.keys().next().value);}
async function readTrendIndex(fileName){if(trendBuffers.has(fileName)){const buffer=trendBuffers.get(fileName);cacheTrendBuffer(fileName,buffer);return buffer;}const directory=await reportDirectory(),file=await(await directory.getFileHandle(fileName)).getFile(),buffer=await file.arrayBuffer(),view=new DataView(buffer);if(buffer.byteLength<12||view.getUint32(0,true)!==TREND_MAGIC||view.getUint32(4,true)!==1||12+view.getUint32(8,true)*12>buffer.byteLength)throw new Error('趋势索引损坏或版本不兼容。');cacheTrendBuffer(fileName,buffer);return buffer;}
...
async function ensureTrend(report,token,reportIndex=0,reportTotal=1){let trendFile=report.trendFile||String(report.storageFile).replace(/\.jsonl\.gz$/,'.trend.bin');try{return{buffer:await readTrendIndex(trendFile),created:null};}catch(error){self.postMessage({type:'trend-migration',token,label:report.label,reportIndex,reportTotal});if(Number(report.storageVersion||1)>=2){const directory=await reportDirectory(),meta=await analysisMeta(report),triples=new Uint32Triples();await scanRows(meta,row=>{const pair=hashPair(row.searchKey);triples.add(pair[0],pair[1],row.rank);},token,'trend-migration-progress',{reportIndex,reportTotal,label:report.label});trendFile=await saveTrendArrays(directory,trendFile,triples,token);}else{const records=await loadLegacyReport(report.storageFile,token,reportIndex,reportTotal,'trend-migration-progress');trendFile=await saveLegacyTrend(report,records,token,trendFile);}trendBuffers.delete(trendFile);return{buffer:await readTrendIndex(trendFile),created:{id:report.id,trendFile}};}}
...
async function scanRows(meta,onRow,token,progressType='query-progress',progress={}){const reader=meta.file.slice(HEADER_SIZE,meta.indexOffset).stream().getReader();let carry=new Uint8Array(0),ordinal=0;while(true){const{value,done}=await reader.read();if(done)break;const data=new Uint8Array(carry.length+value.length);data.set(carry);data.set(value,carry.length);const view=new DataView(data.buffer),rows=[];let offset=0;while(offset+24<=data.length){const length=view.getUint32(offset+20,true);if(offset+24+length>data.length)break;const term=decoder.decode(data.subarray(offset+24,offset+24+length));rows.push({ordinal,rank:view.getUint32(offset,true),term,searchKey:normalize(term),top3Click:view.getFloat64(offset+4,true),top3Conversion:view.getFloat64(offset+12,true)});ordinal++;offset+=24+length;}carry=data.slice(offset);for(const row of rows)onRow(row);if(ordinal&&ordinal%50000<rows.length)self.postMessage({type:progressType,token,count:ordinal,total:meta.count,totalRecords:meta.count,...progress});}if(carry.length)throw new Error('分析数据末尾不完整。');}

class Uint32List{constructor(size=65536){this.buffer=new Uint32Array(size);this.length=0;}push(value){if(this.length===this.buffer.length){const next=new Uint32Array(this.buffer.length*2);next.set(this.buffer);this.buffer=next;}this.buffer[this.length++]=value;}finish(){return this.buffer.slice(0,this.length);}}
...
async function queryPage(primary,comparison,filters,sorts,page,pageSize,queryKey,token){const meta=await analysisMeta(primary),created=[];let comparisonBuffer=null;if(comparison){const ensured=await ensureTrend(comparison,token);comparisonBuffer=ensured.buffer;if(ensured.created)created.push(ensured.created);}const hasFilters=Boolean(filters.keyword)||['rankMin','rankMax','clickMax','conversionMax','changeMin','changeMax'].some(key=>filters[key]!=null);const simpleSort=!sorts.length||(sorts.length===1&&sorts[0].key==='newRank');if(!hasFilters&&simpleSort){const descending=sorts.length&&sorts[0].direction==='desc',start=(page-1)*pageSize,end=Math.min(meta.count,start+pageSize);let ordinals;if(descending){const ascStart=Math.max(0,meta.count-end),ascEnd=Math.max(0,meta.count-start);ordinals=Array.from({length:Math.max(0,ascEnd-ascStart)},(_,i)=>ascEnd-1-i);}else ordinals=Array.from({length:Math.max(0,end-start)},(_,i)=>start+i);const rows=await readRowsByOrdinals(meta,ordinals);for(const row of rows){const pair=hashPair(row.searchKey),oldRank=comparisonBuffer?(lookupRank(comparisonBuffer,pair[0],pair[1])||0):0;row.newRank=row.rank;row.oldRank=oldRank;row.change=oldRank-row.rank;}return{rows,totalCount:meta.count,created};}
  if(!queryCache||queryCache.key!==queryKey){const matchList=new Uint32List(),newRanks=sorts.length?new Uint32Array(meta.count):null,needOld=Boolean(comparisonBuffer)&&(filters.changeMin!=null||filters.changeMax!=null||sorts.some(sort=>sort.key==='oldRank'||sort.key==='change')),oldRanks=needOld?new Uint32Array(meta.count):null;await scanRows(meta,(row)=>{if(newRanks)newRanks[row.ordinal]=row.rank;if(!matchesBaseFilter(row,filters))return;let oldRank=0;if(needOld){const pair=hashPair(row.searchKey);oldRank=lookupRank(comparisonBuffer,pair[0],pair[1])||0;oldRanks[row.ordinal]=oldRank;}if(matchesChangeFilter(row,filters,oldRank))matchList.push(row.ordinal);},token);const matches=matchList.finish();if(sorts.length){if(sorts.length===1&&sorts[0].key==='newRank'){if(sorts[0].direction==='desc')matches.reverse();}else matches.sort((left,right)=>{for(const sort of sorts){let a,b;if(sort.key==='newRank'){a=newRanks[left];b=newRanks[right];}else if(sort.key==='oldRank'){a=oldRanks?oldRanks[left]:0;b=oldRanks?oldRanks[right]:0;}else{a=(oldRanks?oldRanks[left]:0)-newRanks[left];b=(oldRanks?oldRanks[right]:0)-newRanks[right];}if(a!==b)return sort.direction==='asc'?a-b:b-a;}return newRanks[left]-newRanks[right]||left-right;});}queryCache={key:queryKey,matches,newRanks,oldRanks};}
...
async function queryTrends(reports,terms,token){const hashes=terms.map(key=>({key,pair:hashPair(key)})),ranks=Object.fromEntries(hashes.map(item=>[item.key,new Array(reports.length).fill(null)])),created=[];for(let reportIndex=0;reportIndex<reports.length;reportIndex++){const report=reports[reportIndex];let ensured;try{ensured=await ensureTrend(report,token,reportIndex,reports.length);}catch(error){throw new Error(`${report.label}：${error.message||error}`);}if(ensured.created)created.push(ensured.created);for(const item of hashes)ranks[item.key][reportIndex]=lookupRank(ensured.buffer,item.pair[0],item.pair[1]);self.postMessage({type:'trend-progress',token,reportIndex,reportTotal:reports.length,label:report.label});}return{ranks,created};}

self.onmessage=async({data})=>{const{action='parse',file,index,total,token,site,fileHash,storageFile,reports,terms,primary,comparison,filters,sorts,page,pageSize,queryKey}=data;try{
  ...
  if(action==='hash-file'){const sha=new Sha256(),chunkSize=8*1024*1024;let bytes=0,lastPercent=-1;while(bytes<file.size){const end=Math.min(file.size,bytes+chunkSize),value=new Uint8Array(await file.slice(bytes,end).arrayBuffer());sha.update(value);bytes=end;const percent=Math.floor(bytes/Math.max(1,file.size)*100);if(percent!==lastPercent){lastPercent=percent;self.postMessage({type:'hash-progress',token,index,total,percent,bytes});}}self.postMessage({type:'hash-result',token,index,total,fileHash:sha.hex()});return;}
  ...
  const result=await importReport(file,site,token,index,total,fileHash);self.postMessage({type:'complete',token,index,total,...result});
}catch(error){const message=error?.message||String(error)||'未知错误';const type=action==='trend'?'trend-error':action==='query-page'?'page-error':action==='hash-file'?'hash-error':'error';self.postMessage({type,token,index,total,message});}};
